...
system error from `CreateProcessW`. [`Error`] preserves the phase, operation,
and Win32 code. [`CleanupError`] always contains at least one reclamation
failure. Nested Job behavior likewise remains subject to Jobs already imposed
by the host.
...
- Raw attribute injection, creation flags, and mitigation constructors are not
  exposed. Supported creation behavior is selected through [`ConsoleMode`] and
  dedicated [`SpawnOptions`] setters.
...
- With an alternate parent, the resource is duplicated into the effective
  parent's handle table before the child-visible value is lowered. Internal
  handle kinds and handle-table ownership are distinct types.
...
inheritance when transferred handles are sensitive. The crate does not use a
...
library implementing [`AsPseudoConsole`]. [`BorrowedPseudoConsole`] carries the
owner lifetime, so it cannot outlive that terminal object. That library defines
when terminal pipes close and when terminal EOF occurs. Pseudoconsole process
creation sets
...
- Dropping a normal [`Child`] detaches by default, matching
  [`std::process::Child`].
- [`JobClosePolicy::TerminateProcesses`] owns a private innermost Job. Dropping
  the child terminates the root and its descendants.
- [`Child::wait_with_output`] drains stdout and stderr concurrently. With
  `TerminateProcesses`, it terminates remaining descendants after the root
  exits before joining the readers. This guarantees pipe EOF even when a
  grandchild inherited a writer. Both reader threads are joined even when one
  reader fails or panics.
...
Every process is initially created suspended. The private pipeline consumes
`ValidatedPlan`, `PreparedSpawn`, `CreatedSuspended`, and `Reclaimed` in order.
Attribute backing is pinned and owned by its attribute list. Remote duplicates
are reclaimed before the only state that can resume the primary thread exists.
An explicitly suspended request returns [`SuspendedChild`] instead of resuming.

# Compile-time migration checks

Boolean mitigation policies are not accepted.

```rust,compile_fail
use windows_spawn::MitigationPolicy;

let _ = MitigationPolicy::new().dep(true);
```

Raw creation flags are not part of the public API.

```rust,compile_fail
use windows_spawn::{CreationFlags, SpawnOptions};

let _ = SpawnOptions::new().creation_flags(CreationFlags::NEW_CONSOLE);
```

Resuming consumes the suspended state.

```rust,compile_fail
use windows_spawn::{Command, Result};

fn resume_twice(command: &mut Command) -> Result<()> {
    let suspended = command.spawn_suspended()?;
    let _running = suspended.resume()?;
    let _ = suspended.id();
    Ok(())
}
```
...
- [Version 0.2 migration](https://github.com/P4suta/windows-spawn/blob/main/docs/migration-0.2.md)
